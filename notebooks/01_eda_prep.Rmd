...
library(tidyverse) 
library(corrplot) # Necesaria para plot_correlation_matrix
library(GGally)   # Necesaria para plot_pairplot
library(Stat2Data)
...
Si revisamos la fuente oficial del dataset (https://rdrr.io/rforge/Stat2Data/man/Hawks.html) podemos extraer informacion siguiente:
...
Numerical Variables: Variables continuas que representan medidas físicas como Wing, Weight, Culmen, etc.

Note: Species will be on our verdad biologica. Aunque en un trabajo realsta no supervisado, esta NO existiria, aqui la analizaremos en conjunto con el resto de varaibles por lo didactica de este proyecto.
...
Podriamos quedarnos y entrenar con ella, aunque modeloizas con solo una variable no es lo ideal. Asi, para ser conservadores, seleccionaremos tanto la pca1 como la pca2 (esta ultima podria darnos pequeños detalles sutiles) para el entreno, asegurando la vairanza explicada suficiente, sin multicolinealidad. Ademas, el escoger dos pca encaja perfectamente con los modelos que alicaremos (como porfundarizaremos en los siguientes notebooks).
...
Note: It is crucial to acknowledge that the current preprocessing workflow has been intentionally designed to align with the mathematical requirements of the selected algorithms. Different modeling approaches would have necessitated different data preparation strategies.

Specifically, the K-means algorithm is the primary beneficiary of our pipeline. By applying Z-score normalization and PCA transformation, we have created an orthogonal feature space where variables are on a comparable scale and clusters are encouraged to take a "spherical" form. While DBSCAN is inherently more flexible regarding cluster shape and does not strictly require such rigid geometric conditions, both algorithms benefit from the elimination of noise and the reduction of dimensionality.

Utilizing a single, standardized dataset allows for a direct and controlled comparison. Although this framework is optimized for K-means' rigid assumptions, it serves a higher analytical purpose: it enables us to test whether the natural groupings of the hawks are robust enough to be identified by fundamentally different clustering logics (centroid-based vs. density-based) within the same geometric environment.